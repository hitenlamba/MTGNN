...
def plot_predictions(true_values, predicted_values, timestamps):
    ...
    plt.title('Predicted vs True RET Values')
    plt.legend()
    plt.grid(True)
...
def evaluate(data, X, Y, model, evaluateL2, evaluateL1, batch_size):
    ...
    for X, Y in data.get_batches(X, Y, batch_size, False):
        X = X.transpose(1, 2)  # Ensure the input has shape [batch_size, num_channels, num_nodes, seq_in_len]
        with torch.no_grad():
            output = model(X)
...
parser.add_argument('--data', type=str, default='./data/', help='location of the data file')
parser.add_argument('--log_interval', type=int, default=500, metavar='N', help='report interval')
...
parser.add_argument('--num_nodes', type=int, default=7, help='number of nodes/variables')
...
parser.add_argument('--seq_in_len', type=int, default=1, help='input sequence length')
